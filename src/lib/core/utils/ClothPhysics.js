...
class ClothHelper extends Object3D {
	...
	updateMatrixWorld( force ) {
		...
		if ( this.visible ) {
			...
			for ( var i = 0, il = this.physics.spheres.length; i < il; i ++ ) {
				...
				var p =this.physics.convertCoord(this.physics.spheres[i].center, this.physics.spheres[i].handle)
			...
			for ( var i = 0, il = this.physics.cylinders.length; i < il; i ++ ) {
				...
				var p =this.physics.convertCoord(this.physics.cylinders[i].upcenter, this.physics.cylinders[i].handle1)
				var p1 = this.physics.convertCoord(this.physics.cylinders[i].downcenter, this.physics.cylinders[i].handle1)
	...
	_init() {
		...
		for ( var i = 0, il = this.physics.spheres.length; i < il; i ++ ) {
			...
			this.add( new Mesh( createGeometry( {"shapeType":0,"width":this.physics.spheres[i].radius} ), this.materials[ 0 ] ) );

		}

		for ( var i = 0, il = this.physics.cylinders.length; i < il; i ++ ) {
			var cy =  this.physics.cylinders[i];
			this.add( new Mesh( createGeometry( {"shapeType":2,"width":cy.Radius,"height":cy.upcenter.clone().sub(cy.downcenter).length()} ), this.materials[ 0 ] ) );
		...
		gltf.traverse(child => {
            if (child.userData!=undefined && child.userData["dynamic"]!=undefined) {
               ...
               if(!ok){
               		...
               		//scene.add(h);
               		if(this.clothPhysics[gltf]==undefined)
               			this.clothPhysics[gltf] =[];
...
class ClothPhysics {
	...
	constructor( mesh, mainBody,xml ) {
        ...
        if (doc.hasAttribute("constrain")){
            ...
            this.constrainBoneIndex = this.getBoneIndexByName(this.mainBody.skeleton, this.constrainBoneName);
        }

        this.rowcol = true;
        if(doc.hasAttribute("rowcol"))
            this.rowcol = this.parseBoolean(doc.getAttribute("rowcol"));
        ...
        for(var k=0; k<mesh.skeleton.bones.length; k++)
        {
            ...
            if(pair.length == 2) {
             ...
             //   boneCol[bi]++;
                ...
                if(this.mapRow[handle]>this.rownum)
                    this.rownum=this.mapRow[handle];
                if(this.mapCol[handle]>=this.boneCol[bi])
                    this.boneCol[bi]=this.mapCol[handle]+1;
            }
            else {
            }
        }
        ...
        for (var j = 0; j < particles.childNodes.length; j++) 
        {
        	...
        	if(particle.nodeType == Node.TEXT_NODE)
        		continue;
            ...
            var constrain = particle.getAttribute("constrain");
            if(constrain==null)
            	constrain = "";
            if(constrain!="")
                this.mapCons[index] = constrain;
            else
                this.mapCons[index] = this.constrainBoneName;

            const rightParticle = particle.getAttribute("rightParticle");
            if(rightParticle!=""){
            	if(this.mapBone[rightParticle]==undefined)
            		this.yMap[index] =undefined;
            	else
                	this.yMap[index] = parseInt(this.mapBone[rightParticle]);
            }
        ...
        if(upcloths.length>0){
        	...
        	for (var j = 0; j < upcloths.childNodes.length; j++) 
	        {
	        	...
	        	if(upbone.nodeType == Node.TEXT_NODE)
	        		continue;
        ...
        if(links.getAttribute("parentchild"))
            this._buildParentChildLink(mesh.skeleton.bones[0]);
        ...
        if(links.hasAttribute("kdrag"))
            this.kdrag = parseFloat(links.getAttribute("kdrag"));

        if(links.hasAttribute("vdrag"))
            this.vdrag = parseFloat(links.getAttribute("vdrag"));

        if(links.hasAttribute("vdrag_up"))
            this.vdrag_up = parseFloat(links.getAttribute("vdrag_up"));

        if(links.hasAttribute("target"))
            this.TARGETCONSTRAINT=parseBoolean(links.getAttribute("target"));

        if(links.hasAttribute("horizon"))
            this.HORIZON = parseBoolean(links.getAttribute("horizon"));

        if(links.hasAttribute("targetprop"))
            this.targetprop = parseFloat(links.getAttribute("targetprop"));

        if(links.hasAttribute("rowprop"))
            this.rowprop = parseFloat(links.getAttribute("rowprop"));

        if(links.hasAttribute("earth"))
            this.EARTHCOLLISION = parseBoolean(links.getAttribute("earth"));
        if(links.hasAttribute("jack"))
            this.JACKCLOTH = parseBoolean(links.getAttribute("jack"));
        ...
        if(links.hasAttribute("split"))
            this.SPLITCLOTH = parseBoolean(links.getAttribute("split"));
...
　　				const n = this.yMap[this.yMap[key]];
        ...
        if(links.hasAttribute("rownum"))
            this.rownum = parseInt(links.getAttribute("rownum"));

        if(links.hasAttribute("rownum2"))
            this.rownum2 = parseInt(links.getAttribute("rownum2"));
	...
	updateBone(){
		...
		this.calcBoneTransform();
		for(var i=0; i<nBone; i++){
			if(this.upcloth[i]||i==0||this.invmass[i]==0)
				continue;
			const bone = bones[i];
			...
			if (bone.parent ){
				...
				bone.quaternion.set(parentQuatInvert.x, parentQuatInvert.y, parentQuatInvert.z,parentQuatInvert.w);
	...
	strimBoneName(boneName){
        ...
        if(pair2.length==3){
        	if(boneName.includes("one")||boneName.includes("two")||boneName.includes("three")||boneName.includes("four")||boneName.includes("five")||boneName.includes("six")||boneName.includes("seven")||boneName.includes("eight")||boneName.includes("zero"))
	...
	loadSolid(s,  start_bias,  end_bias,
                     radius,  s_e_bone)
    {
        ...
        if(s.hasAttribute("end_bias_x"))
            end_bias.x = parseFloat(s.getAttribute("end_bias_x"))/100;

        if(s.hasAttribute("end_bias_y"))
            end_bias.y = parseFloat(s.getAttribute("end_bias_y"))/100;

        if(s.hasAttribute("end_bias_z"))
            end_bias.z =  parseFloat(s.getAttribute("end_bias_z"))/100;

        if(s.hasAttribute("radius"))
            s_e_bone.radius = parseFloat(s.getAttribute("radius"))/200;

        if(s.hasAttribute("start_bone"))
            s_e_bone.s = s.getAttribute("start_bone");

        if(s.hasAttribute("end_bone"))
            s_e_bone.e = s.getAttribute("end_bone");
    }
	addSolids(dom)
    {
         	...
         	for (var j = 0; j < spheres.childNodes.length; j++) 
        {
			...
			var sc={"center":bone_pos, "radius":s_e_bone.radius, "handle":handle, "name":s_e_bone.s,"isAuto":false};
        ...
        if(cylinders!=undefined)
        {
           	...
           	for (var j = 0; j < cylinders.childNodes.length; j++) 
        	{
                ...
                ///sumingnan
                if (s_e_bone.s == leftchestName)
                    isExistLeft = true;
                else if (s_e_bone.s == rightchestName)
                    isExistRight = true;
                ...
                this.cylinders.push({"upcenter":start_bone_pos, "downcenter":end_bone_pos,  "Radius":s_e_bone.radius, "Length":len, "handle1":handle, "handle2":handle2, "name":s_e_bone.s, "downname":s_e_bone.e, "isAuto":isExistLeft || isExistRight});
            }
            ///sumingnan
            if (this.constrainBoneName == "head")
            {
                if (!isExistLeft)
                {
                    ...
                    this.cylinders.push({"upcenter":bone_pos, "downcenter":end_bone_pos, "Radius":chestRadius, "Length":len, "handle1":handle, "handle2":handle2, "name":leftchestName, "downname":"Leftchest", "isAuto":true});
                }
                if (!isExistRight)
                {
                    ...
                    this.cylinders.push({"upcenter":bone_pos,"downcenter":end_bone_pos, "Radius":chestRadius,"Length":len, "handle1":handle, "handle2":handle2, "name":rightchestName,"downname":"rightchest", "isAuto":true});
                }
            }
	...
	_buildParentChildLink(bone)
    {
        for(var i=0; i<bone.children.length; i++){
            ...
            if(child!=null){
                ...
                this.AddLink(this.mapBone[this.strimBoneName(bone.name)], this.mapBone[this.strimBoneName(child.name)], 1.0);
      ...
      // this.mainBody.skeleton.getBoneByName("rightUpLeg").updateWorldMatrix(true, false);
        ...
        if(this.resetPos)
        {
            ...
            for(var k=0; k<this.pos.length; k++)
            {
                if(this.JACKCLOTH && !this.upcloth[k])
                    ...
                    this.targetPosition[k] = this.convertCoord(this.initPosition[k], this.mapConsIndex[k]);
       ...
       /* Update positions. */
        if(this.cylinders.length!=0)
            this.clothFixMove(this.cylinders[0].Radius);
        ...
        for (var i = 0; i <  this.pos.length; i++) {
            ...
            if(this.invmass[i]==0 || this.upcloth[i])
                continue;
            this.old_pos[i].copy(this.pos[i]);
            this.pos[i].add( new Vector3().copy(this.velo[i]).multiplyScalar((1-airdrag)).add(new Vector3().copy(this.gravity).multiplyScalar(dt*dt)).add(new Vector3().copy(this.accel[i]).multiplyScalar(dt*dt)));
        ...
        if (this.SQUARECOLLISION)
			this.secondRowAdjust(this.targetPosition);
        ...
        for(var count=0; count<this.iter; count++)
        {
            ...
            for(var k=0; k<this.pos.length; k++) {
              ...
              //      pos[k] = pos[k]*(1-sf) + targetPosition[k]*sf;
                ...
                this.pos[k] = this.pos[k].multiplyScalar(1-sf).add(this.targetPosition[k].clone().multiplyScalar(sf));
        	    //this.pos[k].copy(this.targetPosition[k]);
            }
        }
        ...
        for(var i=0; i<this.pos.length; i++)
            this.velo[i].copy(this.pos[i]).sub(this.old_pos[i]);
	...
	clothFixMove( Radius)
    {
        ...
        for(var j=0; j<this.boneCol[2]; j++) {  // move other particles with the moving of fixed particles;
        	
            
        	if(this.boneHandle[2][j]==-1)
        		continue;
            ...
            for(var i=0; i<=rowall; i++) {
                if(this.boneHandle[i][j] == -1)
                    continue;
                if(this.invmass[this.boneHandle[i][j]] == 0)
                    continue;
                if(movelen > cr)
                    this.pos[this.boneHandle[i][j]].add(move.clone().sub(moveunit.clone().multiplyScalar(cr)));
                //                    else
	...
	springRestlengthReset(targets)
	{
		for (var i = 0; i < this.constraints.length; i++)
		{
			...
			if (this.upcloth[a] || this.upcloth[b])
				continue;
			if (!this.invmass[a] && !this.invmass[b])
				continue;
	...
	secondRowAdjust(targets)
	{
		if(this.rownum < 2)
			return;
		if(this.constrainBoneName != "hips1")
			return;
		const second = 2;
		...
		for(var j=0; j<this.boneCol[second]; j++)
		{
			...
			for(var i=second+1; i<n; i++)
			{
			    ...
			    targets[p].copy(targetLine.clone().multiplyScalar(length*lengthRefer).add(targets[rn]));		
			}
		}
		...
		for(var i=0; i<this.boneCol[second]; i++)
		{
			...
			if(cosThird < cosSecond){
			    targets[p]= thirdDir.clone().multiplyScalar(targets[p].clone().sub(cPos).dot(thirdDir)).add(cPos);
			}
		}
	}
	solveCollision(upcenters, downcenters, centers,  horizon)
    {
        ...
        for(var i=0; i<movedirect.length; i++)
        	movedirect[i] = new Vector3();
	...
	//		secondMove = false;
        ...
        for(var j=0; j<this.constraints.length; j++) {
			...
			if (this.constraints[j].soft != -1.0 && this.constraints[j].soft != 1.0)
				continue;
            if(horizon)
                if(/*constraints[j].soft!=1.0 &&*/ this.constraints[j].soft!=-1.0) //not the row or column spring
                    continue;
            if(this.upcloth[p1] || this.upcloth[p2])
                continue;
            if(!secondMove && this.invmass[p1] == 0 || this.invmass[p2] == 0 )
                continue;
			...
			var m2 = secondMove ? 1 : this.invmass[p2];
             for(var i=0; i<this.cylinders.length; i++) {         // solve cylinder collision;
         	...
         	//			move_point_outside_cylinder(pos[p2], upcenters[i], downcenters[i], cylinders[i].Radius);
         				this.move_segment_outside_cylinder(this.pos[p1], this.pos[p2], axisdirection[i], upcenters[i], downcenters[i], this.cylinders[i].Radius, m1, m2);
             }
			 ...
			 /*	move_point_outside_sphere(pos[p1], centers[i], spheres[i].radius);
			 	...
			 	this.move_segment_outside_sphere(this.pos[p1], this.pos[p2], m1, m2, centers[i], this.spheres[i].radius);
             }
	...
	stabilityConstrain(targets)
    {
        ...
        _upDir = this.convertCoord(_upDir, this.constrainBoneIndex).sub(this.GetBonePos(this.constrainBoneName));
        if(_upDir.y < 0)
            return;
        ...
        const legPos=new Array(2), legDir=new Array(2);
        ...
        for(var l=0; l<2; l++){
            ...
            for(var i=2; i<rowall; i++){
                for(var j=0; j<this.boneCol[i]; j++){
                    ...
                    if(idx==-1 || this.invmass[idx]==0 || (targets[idx].clone().sub(legPos[l]).dot(legDir[l])>0))
                        continue;
	 ...
	 CONSTRAIN_FIX_MOVE(pfixed, pmoving, dest_length){
	 	...
	 	pmoving.copy(new Vector3().copy(pfixed).add(new Vector3().copy(pmoving).sub(pfixed).normalize().multiplyScalar(dest_length)));
	 }
	 GetBonePos(boneName){
	 	var pos= new Vector3();
    ...
    {
        if(this.cylinders.length==0 /*|| rownum2==0*/)
            return;
        ...
        for(var i=2; i<this.rowall; i++){
            ...
            for(var j=0; j<this.boneCol[i]; j++){
                ...
                if(dis > maxDis){
                    ...
                    if(move > moveMax)
                        move = moveMax;
                    this.pos[idx].add(targets[idx].clone().sub(this.pos[idx]).multiplyScalar(move/dis));
                }
            }
        }
    }
    ...
    solveCollisionSquare( ucenters,  dcenters)
    {
        ...
        var directs=[], upps=[], downps=[], uRadius=[], dRadius=[];
        var movedirect = new Array(this.pos.length);
        for(var i=0; i<movedirect.length; i++)
        	movedirect[i] = new Vector3();
    ...
    //    std::vector<bool> rotation;
        ...
        for(var i=0; i<cNum; i++)
        {
            ...
            if(this.squareList[i].length == 2)
                needRotate = true;
            else if(this.squareList[i].length == 1)
                needRotate = false;
            else
                continue;
            var dir1, dir2;
            ...
            if(needRotate){
                ...
                rdownc = dcenters[downidx].clone().sub(dcenters[upidx]).applyQuaternion(legRotate).add(dcenters[upidx]);
            }
            else{
                dRad = this.cylinders[upidx].Radius;
            }
        ...
        //    uLen = lengths[upidx];
            ...
            for(var j=0; j<this.boneCol[2]; j++){
                ...
                if(idx!=-1)
                    referSquare[idx] = this.convertCoord(this.initPosition[idx], this.cylinders[this.squareList[i][0]].handle1);
            }
            ...
            for(var ic=0; ic<this.constraints.length; ic++){
                ...
                if(this.boneHandle[r1][c1]==-1 || this.boneHandle[r2][c2]==-1 || this.invmass[this.boneHandle[r1][c1]]==0 || this.invmass[this.boneHandle[r2][c2]]==0)
                    continue;
                const square = new Array(4), projsqr=new Array(4), directions=new Array(4);
                ...
                if(needRotate && this.mapRow[p3] >= this.rownum){
                    const p2m = square[2].clone().sub(pm), q2m = square[3].clone().sub(pm), s02m = square[0].clone().sub(pm), s12m = square[1].clone().sub(pm);
                    const upCos0 = s02m.dot(dir1), upCos1 = s12m.dot(dir1);
                    const downCos0 = s02m.dot(dir2), downCos1 = s12m.dot(dir2);
					/*if(upCos0 < downCos0)
                ...
                //    q2m = square[3] - pm;
                    if(p2m.dot(dir1)<p2m.dot(dir2) || q2m.dot(dir1)<q2m.dot(dir2)){
                        ...
                        square[2].copy(square[2].clone().sub(pm).applyQuaternion(legRotateInvert.copy(legRotate).invert()).add(pm));
                        square[3].copy(square[3].clone().sub(pm).applyQuaternion(legRotateInvert.copy(legRotate).invert()).add(pm));
				...
				if(solve != 0)
					square[2].copy(square[2].clone().sub(this.pos[p2]).multiplyScalar(rest/solve).add(this.pos[p2]));
				rest = tempPos[p4].clone().sub(tempPos[p1]).length();
				solve = square[3].clone().sub(this.pos[p1]).length();
				if(solve != 0)
					square[3].copy(square[3].clone().sub(this.pos[p1]).multiplyScalar(rest/solve).add(this.pos[p1]));
    ...
    pushVerticlesOutside( square,  upcenter,  downcenter,  low,  Rad, rc)
    {
        ...
        const directions = new Array(4), projsqr = new Array(4);
        direct.normalize();
        ...
        var center = direct.clone().multiplyScalar(projsqr[3].clone().sub(upcenter).dot(direct)).add(upcenter);
        for(var n=0; n<square.length-1; n++){
            projsqr[n]=direct.clone().multiplyScalar(projsqr[3].clone().sub(square[n]).dot(direct)).add(square[n]);
        }
        directions[0] = projsqr[0].clone().sub(center).cross(projsqr[square.length-1].clone().sub(center));
        for(var n=1; n<square.length; n++){
            directions[n] = projsqr[n].clone().sub(center).cross(projsqr[n-1].clone().sub(center));
        }
	...
	//	move_point_outside_sphere(projsqr[3], center, Rad);
        ...
        moveunit.normalize();
        var t = (center.clone().sub(projsqr[3]).dot(moveunit))/(projsqr[2].clone().sub(projsqr[3]).length());
        t = clamp(t, 0.0, 1.0);
        ...
        if(notThrough){
            if(verLen < Rad)
                ver = ver.clone().multiplyScalar(Rad - verLen);
            else
                return false;
        }
        else
            ver = ver.clone().multiplyScalar(-1).multiplyScalar(verLen+Rad); //ver.clone().multiplyScalar(verLen + Rad).multiplyScalar(-1);
    ...
    constrain()
    {
        ...
        for (var k = 0; k < max_constrain_iters; k++)
        {
            ...
            for(var l=0; l<this.constraints.length; l++){
                ...
                if(this.invmass[c.particleA] == 0 && this.invmass[c.particleB]==0)
                    continue;
                if(c.soft!= 1.0)
                    continue;
                ...
                for(var sp=0; sp<centers.length;sp++)
                    this.move_segment_outside_sphere(this.pos[c.particleA], this.pos[c.particleB],this.invmass[c.particleA], this.invmass[c.particleB] ,centers[sp], this.spheres[sp].radius);
            }
    ...
    move_segment_outside_sphere(a, b,massa,massb,center,  radius) {
        ...
        const a0 = new Vector3().copy(a).sub(center), b0 = new Vector3().copy(b).sub(center);
 ...
 //       double t = -(a0.dotProduct(v))/v.length();
    ...
    move_segment_outside_cylinder(a, b,  direct,  up, down,  radius,  ma,  mb)
    {
        ...
        {
          const p = up.clone();
           if(ver.clone().sub(down).dot(direct)<0) 
            	p.copy(down);
    ...
    springConstrain(targets)
    {
        var n = new Vector3();
        var p1, p2, r1, r2;
        var d, l , templength, f=1.0, damp=0.5;
        const referRestLength={}, referRestLength2={};
        if(this.rownum2!=0){
            for(var i=0; i<this.constraints.length; i++){
                if(this.constraints[i].soft==-1 && this.mapRow[this.constraints[i].particleA]==this.rownum-1){
                    const col = this.mapCol[this.constraints[i].particleA];
                    referRestLength[col] = this.constraints[i].restLength;
                }
				else if (this.constraints[i].soft == 0 && this.mapRow[this.constraints[i].particleA] == this.rownum - 1) {
					const col = this.mapCol[this.constraints[i].particleA];
        ...
        for(var i=0; i<this.constraints.length; i++)
        {
            ...
            if(this.invmass[p1] ==0 && this.invmass[p2]==0)
                continue;
            if(this.upcloth[p1] && this.upcloth[p2])
                continue;
            ...
            if(this.constraints[i].soft == 1.0)  //column spring;
            {
                ...
                if( templength > c.restLength)
                {
                    ...
                    if(this.invmass[c.particleA]!=0)
                        this.pos[c.particleA].add(new Vector3().copy(n).multiplyScalar(damp*(templength-c.restLength)));
                }

            }

            else if (this.constraints[i].soft == -1.0 || this.constraints[i].soft == -2.0) // row spring;
            {
                f = this.rowprop;
                const row = this.mapRow[p1];
                if(this.MIDDLEPROCESS || (!this.JACKCLOTH && !this.SPLITCLOTH && this.mapCons[p1]!=this.mapCons[p2])){
                    l = this.constraints[i].restLength = new Vector3().copy(targets[p1]).sub(targets[p2]).length();
                }
                ...
                if(this.rownum2!=0 && row>=this.rownum)
                    if(l < referRestLength[this.mapCol[p1]])
                        l = referRestLength[this.mapCol[p1]];
                d -= l;
			...
			else if (this.constraints[i].soft == 0) // 
			{
				...
				n.normalize();
				if (this.rownum2 != 0 && row >= this.rownum)
					if (l < referRestLength2[this.mapCol[p1]])
						l = referRestLength2[this.mapCol[p1]];
				d -= l;
				if (this.invmass[p1] != 0)
					this.pos[p1].add(new Vector3().copy(n).multiplyScalar(f*d/2.0));
				if (this.invmass[p2] != 0)
