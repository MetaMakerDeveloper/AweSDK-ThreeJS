...
class ClothPhysicManager {
  ...
  setClothPhysics(gltf, scene) {
    ...
    if (body == undefined || !(body instanceof Mesh)) body = gltf.getObjectByName("head_part");
    gltf.traverse((child) => {
...
class ClothPhysics {
  ...
  constructor(mesh, mainBody, xml) {
    ...
    for (var j = 0; j < particles.childNodes.length; j++) {
      ...
      else this.mapCons[index] = this.constrainBoneName;
  ...
  updateBone() {
    ...
    this.calcBoneTransform();
    for (var i = 0; i < nBone; i++) {
      if (this.upcloth[i] || i == 0 || this.invmass[i] == 0) continue;
      const bone = bones[i];
